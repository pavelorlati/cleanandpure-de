          ...
          <div className="container-x grid lg:grid-cols-2 gap-16">
            ...
            <form onSubmit={onSubmit} className="bg-card/60 border border-border/60 p-8 rounded-sm space-y-4 backdrop-blur-sm">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <input required type="email" value={form.email} onChange={(e)=>setForm({...form, email: e.target.value})} placeholder="E-Mail-Adresse *" className="w-full px-4 py-3 bg-background border border-border rounded-sm text-sm focus:outline-none focus:border-primary" />
              </div>
              <input value={form.tel} onChange={(e)=>setForm({...form, tel: e.target.value})} placeholder="Telefon" className="w-full px-4 py-3 bg-background border border-border rounded-sm text-sm focus:outline-none focus:border-primary" />
              <select value={form.leistung} onChange={(e)=>setForm({...form, leistung: e.target.value})} className="w-full px-4 py-3 bg-background border border-border rounded-sm text-sm focus:outline-none focus:border-primary">
                <option value="">Leistung wählen</option>
                {leistungen.map((l) => <option key={l} value={l}>{l}</option>)}
              </select>
              <textarea required value={form.nachricht} onChange={(e)=>setForm({...form, nachricht: e.target.value})} placeholder="Ihre Nachricht *" rows={6} className="w-full px-4 py-3 bg-background border border-border rounded-sm text-sm focus:outline-none focus:border-primary" />
